...
import { useScreenSaverContext, useStateContext } from "@/lib/functions";
import { durationOptions } from "@/lib/providers";
...
function CubePage() {
  ...
  useEffect(() => {
    ...
    if (!cubeTutorial) {
      driverObj2.drive();
      localStorage.setItem("cubeTutorial", "true");
    }
  }, []);

  return (
    <>
      <div className="w-full flex flex-col items-center bg-background pt-[85px] sm:pt-[105px]  px-5 sm:px-[60px] max-w-[2000px] min-h-[calc(100vh-85px)] sm:min-h-[calc(100vh-105px)]">
